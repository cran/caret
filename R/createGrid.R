...
function(method, len = 3, data = NULL)
{
   ...
   {
      library(rpart)
      initialFit <- rpart(.outcome ~ ., data, control = rpart.control(cp = 0, minsplit = 5))$cptable
      ...
      {
         cat("note: only", dim(initialFit)[1], "possible values of the max tree depth from the initial fit.\n",
            "Truncating the grid to",  dim(initialFit)[1], ".\n\n")
