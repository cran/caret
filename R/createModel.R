...
function(data, method, tuneValue, obsLevels, ...)
{
   ...
   # character and then create factos form them with levels originally
   ...
   modelFit <- switch(method,
      ...
      svmpoly = 
      {
         ...
         if(type == "Classification")
         {
            out <- ksvm(as.matrix(trainX), trainY, kernel = polydot(degree = tuneValue$.degree, scale = tuneValue$.scale, offset = 1), C = tuneValue$.C, prob.model = TRUE, ...)
         } else out <- ksvm(as.matrix(trainX), trainY, kernel = polydot(degree = tuneValue$.degree, scale = tuneValue$.scale, offset = 1), C = tuneValue$.C, ...)
         out            
      },
      svmradial = 
      {      
         ...
         if(type == "Classification")
         {
            out <- ksvm(as.matrix(trainX), trainY, kernel = rbfdot(sigma = tuneValue$.sigma), C = tuneValue$.C, prob.model = TRUE, ...)
         } else {
            out <- ksvm(as.matrix(trainX), trainY, type = "eps-svr", kernel = rbfdot(sigma = tuneValue$.sigma), C = tuneValue$.C, ...)
         }
         out         
      },                          
      nnet =
