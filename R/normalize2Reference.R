...
function (data, refData = NULL, ties = TRUE) 
{
   # adapted from limma:::normalizeQuantiles
    ...
    for (j in 1:numSamples) 
    {
        ...
        sortedSampleI <- sort(data[, j], method = "quick", index.return = TRUE)
        if (ties) dataRanks[, j] <- rank(data[, j])
        nobsj <- length(sortedSampleI$x)
        if (nobsj < numProbes) {
            nobs[j] <- nobsj
            isna <- is.na(data[, j])
            sortedData[, j] <- approx((0:(nobsj - 1))/(nobsj - 1), sortedSampleI$x, 
                quantProbs, ties = "ordered")$y
    ...
    # the row means form the reference quantiles when no explicit reference is given.
    # for example, to get the reference median when no explicit reference is provided, 
    # we take the average median across samples:  mean(apply(sortedData, 2, median))
        ...
        if (nobs[j] < numProbes) {
            ...
            if (ties) 
               ...
               # It returns an approximated mean value 
                data[!isna, j] <- approx(quantProbs, refQuantile, 
                  (dataRanks[!isna, j] - 1)/(nobs[j] - 1), ties = "ordered")$y
            else data[dataOrder[!isna, j], j] <- approx(quantProbs, refQuantile, 
               (0:(nobs[j] - 1))/(nobs[j] - 1), ties = "ordered")$y
        }
        else {
            if (ties) 
                data[, j] <- approx(quantProbs, refQuantile, 
                  (dataRanks[, j] - 1)/(numProbes - 1), ties = "ordered")$y
