...
+     (1:10)), trControl = ctrl)
> modelFree <- filterVarImp(mdrrDescr, mdrrClass)
> allImp2 <- data.frame(randomForest = varImp(rfFit2)[, 1], PLS = varImp(plsFit2$finalModel)[, 
+     1], ROC = modelFree[, 1])
> allImp2$note <- "|Corr| < 0.90"
> allPred <- splom(~allImp1[, 1:3], type = c("p", "smooth"), main = "Variable Importance: All Predictors")
> filteredPred <- splom(~allImp2[, 1:3], type = c("p", "smooth"), 
+     main = "Variable Importance: |Corr| < 0.90 Filter")
\end{Sinput}
\end{Schunk}
