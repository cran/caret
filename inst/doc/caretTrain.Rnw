...
\begin{small}
<<setup>>=
fitControl <- trainControl(method = "LGOCV", p = .75, number = 30, verboseIter = FALSE)
set.seed(2)
@
\end{small}
...
\begin{itemize}
	...
	\item [] The same is also true for nearest shrunken centroid models, where an initial model is fit to find the range of possible threshold values.
   ...
   \item For some models, such as \texttt{ctree} or MARS (via the \texttt{earth} function), this package is inefficient because the same model is needlessly fit several times. For example, for \texttt{earth}, the largest model can be fit without pruning to get performance for the full model. The same model object can be passed to the \texttt{update} function to get performance on sub-models without refitting the entire model. This is a feature that should be in the next version.
...
Note that \texttt{caret} picks the ``best'' values of the tuning parameters based on resampling measures and does not use a one standard deviation rule (like recursive partitioning models). In many cases, the user will need to look at the profiles of performance measures across the tuning parameters to get an idea of the truly best choice.
...
\begin{small}
<<bhModels>>=
rpartFit <- train(trainBH, trainMedv, "rpart", tuneLength = 9, trControl = trainControl(verboseIter = FALSE))
marsFit <- train(trainBH, trainMedv, "mars", trControl = trainControl(verboseIter = FALSE))
rfFit <- train(trainBH, trainMedv, "rf", trControl = trainControl(verboseIter = FALSE, method = "oob"))
@
\end{small}
...
\begin{small}
<<bhPrediction>>=
bhPredictions <- extractPrediction(list(rpartFit, marsFit, rfFit), testX = testBH, testY = testMedv)
...
\begin{figure}
   \begin{center}      
<<bbbPredPlot, echo = FALSE, fig = TRUE>>=
trellis.par.set(caretTheme(), warn = FALSE)
print(plotObsVsPred(bhTestPred))
@         
...
\end{document}
   ...
   options(width = 80)
   Sweave(
      file = "c:/program files/R/R250/bin/caret082007/inst/doc/caretTrain.Rnw",
      out = "c:/program files/R/R250/bin/caret082007/inst/doc/caretTrain.tex")
