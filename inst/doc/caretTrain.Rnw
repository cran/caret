...
\item [] Alternate model training schemes can be used by passing
  ...
  function. Also, using \texttt{method = 'gcvEearth'} will use the
     ...
     Boosted Trees &
      ...
      &
         \texttt{bstTree} & 
            ...
            \texttt{maxdepth}, \texttt{mstop}, \texttt{nu} & \\                   
...
\rowcolor[rgb]{.95, .95, .95}         
       &               
         \texttt{avNNet} & 
            {\tt \href{http://cran.r-project.org/web/packages/caret/index.html}{caret}},
            {\tt \href{http://cran.r-project.org/web/packages/nnet/index.html}{nnet}}&       
            \texttt{decay}, \texttt{size}, \texttt{bag} & \\                  
      ...
      Quantile Regression Forests &
        ...
        \texttt{qrnn} &
         {\tt \href{http://cran.r-project.org/web/packages/qrnn/index.html}{qrnn}}    & 
            \texttt{n.hidden}, \texttt{penalty}, \texttt{bag}  & \\             
                   
 \rowcolor[rgb]{.95, .95, .95}   
      Rule--Based Models &
         \texttt{M5Rules} & 
            {\tt \href{http://cran.r-project.org/web/packages/RWeka/index.html}{RWeka}}    & 
            \texttt{pruned}, \texttt{smoothed}   & \\             
            
 \rowcolor[rgb]{.95, .95, .95}   
       &
         \texttt{M5} & 
            {\tt \href{http://cran.r-project.org/web/packages/RWeka/index.html}{RWeka}}    & 
            \texttt{pruned}, \texttt{smoothed}, \texttt{rules}    & \\   
            
 \rowcolor[rgb]{.95, .95, .95}   
      &
         \texttt{cubist} & 
            {\tt \href{http://cran.r-project.org/web/packages/Cubist/index.html}{Cubist}}    & 
            \texttt{committees}, \texttt{neighbors}  & \\    
...
\begin{itemize}
\item \texttt{method}: The resampling method: \texttt{boot}, \texttt{boot632},
  \texttt{cv}, \texttt{LOOCV}, \texttt{LGOCV}, \texttt{repeatedcv}  and \texttt{oob}. The
...
@
\end{small} 
To obtain predictions  for the MARS model, \texttt{predict.earth} can be
used. 
...
@
\end{small}
Note that the \texttt{plsFit} object used pre--processing. In this
case, we cannot directly call \texttt{predict.mvr} and expect to get
the same answers as \texttt{predict.train}. The latter function knows
that centering and scaling is required and execute these calculations
on the new samples, whereas \texttt{predict.mvr} does not. For the
\texttt{pls} function, there is an argument called \texttt{scale} that
...
using \texttt{confusionMatrix.train}. For each resampling iteration,
...
normalizing these values. See \texttt{?confusionMatrix.train} for details.
...
\begin{figure}
   \begin{center}		
      ...
      \caption{ Examples of output from \texttt{xyplot(resamps, models = c("CART", "MARS"))}. }
