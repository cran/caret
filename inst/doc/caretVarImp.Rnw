...
\begin{itemize}
  ...
  \item {\bf Multivariate Adaptive Regression Splines}: MARS models 
        ...
        Also, the option \texttt{varImp(object, value = "grsq")} 
        compares the GCV statistic for each model to the intercept 
        only model. However, there are some cases when terms are retained 
        in the model that result in an increase in GCV. Negative variable 
        importance values for MARS are set to a small, non-zero number. 
        Alternatively, using
...
plsFit <- train(mdrrDescr, mdrrClass, "pls", tuneGrid = data.frame(.ncomp = 2 * (1:10)), trControl = ctrl)
modelFree <- filterVarImp(mdrrDescr, mdrrClass)

allImp1 <- data.frame(
   randomForest = varImp(rfFit)[,1], 
   PLS = varImp(plsFit$finalModel)[,1], 
   ROC = modelFree[,1])
...
plsFit2 <- train(mdrrDescr, mdrrClass, "pls", tuneGrid = data.frame(.ncomp = 2 * (1:10)), trControl = ctrl)
modelFree <- filterVarImp(mdrrDescr, mdrrClass)

allImp2 <- data.frame(
   randomForest = varImp(rfFit2)[,1], 
   PLS = varImp(plsFit2$finalModel)[,1], 
   ROC = modelFree[,1])
...
<<mbrrImp, echo = FALSE, results = hide>>=

pdf(paste(getwd(), "/plsVarImp.pdf", sep = ""), width = 4.5, height = 7)
   trellis.par.set(caretTheme(), warn = FALSE)
   plot(varImp(plsFit2, scale = FALSE), top = 15, main = "PLS Importance - Top 15")
dev.off()
pdf(paste(getwd(), "/rocVarImp.pdf", sep = ""), width = 4.5, height = 7)
   trellis.par.set(caretTheme(), warn = FALSE)
   plot(varImp(plsFit2, useModel = FALSE, scale = FALSE), top = 15, main = "ROC Importance - Top 15")
dev.off()
pdf(paste(getwd(), "/allPred.pdf", sep = ""), width = 7, height = 7)
   trellis.par.set(caretTheme(), warn = FALSE)
   plot(allPred)
dev.off()
pdf(paste(getwd(), "/filteredPred.pdf", sep = ""), width = 7, height = 7)
   trellis.par.set(caretTheme(), warn = FALSE)
   plot(filteredPred)
dev.off()
@
...
\end{document}
   ...
   options(width = 80)
   Sweave(
      file = "c:/program files/R/R260dev/bin/caret/inst/doc/caretVarImp.Rnw",
      out = "c:/program files/R/R260dev/bin/caret/inst/doc/caretVarImp.tex")
