...
@
      ...
      \caption{Parallel coordinate of the bootstrapped $R^2$ values
        from the random forests model with univariate filters and the RFE models using linear least squares and random forests. These plots were generated using \texttt{parallel(bootValues, metric = "Rsquared")}}
      \label{F:resamps1} 
    \end{center}
\end{figure}
