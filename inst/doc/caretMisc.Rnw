...
There are a few data sets included in \texttt{caret}. The first three
...
\texttt{QSARdata} R pacakge. The fourth data set listed is a typical spectroscopy data set from chemometrics. The last data set is a pattern recognition problem with a large number of classes, some of which do not have many observations.
