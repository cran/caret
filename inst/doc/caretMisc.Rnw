...
The two functions \texttt{processData} and \texttt{applyProcessing} are wrappers for \texttt{scale} that can be used to repeat the same centering and scaling to new sets of data.

In the example below, the half of the MDRR data are used to estimate the location and scale of the predictors. The function \texttt{processData} doesn't actually pre--process the data. \texttt{applyProcessing} is used to pre--process this and other data sets.
\begin{small}
<<centerScale>>=
set.seed(96)
...
\end{document}
   ...
   setwd("c:/program files/R/R250/bin/caret082007/inst/doc/")
   oldWidth <- options("width")$width
   options(width = 80)
   Sweave(
      file = "c:/program files/R/R250/bin/caret082007/inst/doc/caretMisc.Rnw",
      out = "c:/program files/R/R250/bin/caret082007/inst/doc/caretMisc.tex")
